...
4. **Keep authentication user-managed:** if manual enhancement is selected, instruct the user to complete any login or challenge themselves in their own visible browser, then confirm readiness and the selected sources. Never ask for, receive, store, or handle usernames, passwords, MFA codes, cookies, account credentials, or other authentication secrets. Never bypass protection. Use only content actually visible in that browser.
5. **Run automatic visual calibration:** visit the selected automatic sources without login. A source counts only when actual image content or video playback is visible in this run. Page-only access, text-only results, search snippets, thumbnails, metadata, or remembered site style do not count. Skip failures and record the attempted URLs, exact limitation, failure reason, and absence of usable samples.
