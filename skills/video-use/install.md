...
## Shining-Moments-Cut bundle layout

When this file is located at `Shining-Moments-Cut/skills/video-use/install.md` or an agent has already installed that complete `video-use/` directory, treat the directory containing this file as the stable Video Use checkout:

- Do not clone or symlink a second copy from the upstream repository.
- Install dependencies in this directory with `uv sync` or `pip install -e .`.
- Keep `SKILL.md`, `helpers/`, `pyproject.toml`, and `skills/manim-video/` together.
- Store `ELEVENLABS_API_KEY` in the environment or this directory's uncommitted `.env`.
- If the host already discovers this `SKILL.md`, skip the standalone registration step below.

The clone and symlink instructions below remain for standalone upstream installs only.
...
Skip this step for the bundled layout described above; use the existing `skills/video-use/` directory as the stable path.
