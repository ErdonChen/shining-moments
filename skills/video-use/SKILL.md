...
First-time install lives in `install.md` (bundle/standalone path, deps, ffmpeg, skill registration when needed, API key). When this Skill is bundled under `Shining-Moments-Cut/skills/video-use/`, use that directory directly and do not clone a second upstream copy. Don't re-run installation every session; on cold start just verify:
