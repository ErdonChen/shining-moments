...
Shining Moments 是一次“第一次智能过滤”：它帮助缩小复筛范围，但把最终选择权留给用户。
...
一个面向个人照片与视频的智能初筛 Skill。它先排除明确不可用的素材，再把主选、可用备选、重复项和具有亲情或友情价值的瞬间留给用户复筛，而不是替用户做最终决定。
...
开始前先确认原素材只读；Skill 随后按下面四步完成设置：

| 步骤 | 用户选择 | Skill 的处理 |
|---|---|---|
| **1. 题材** | 综合/混合、风景旅行、建筑空间、人文纪实、人像、家庭、朋友、Vlog/活动或自定义 | 按题材采用对应的审美、叙事与关系标准；未选择时默认“综合/混合”。 |
| **2. 素材类型** | 照片、视频或照片 + 视频 | 照片与视频分别校准和筛选。 |
| **3. 参考校准** | 使用自动来源，或再增加一个人工增强来源 | 自动照片来源为 Commons、Flickr、Google 图片；自动视频来源为 Commons、Google 视频。需要时再从人机验证、用户登录或自定义网址中选择一种增强。 |
| **4. 交付方式** | 建立链接（默认）或轻量副本 | 链接最省空间；轻量副本为 RAW 生成/复用 JPEG，并只导出入选的视频片段。两种方式都不移动、覆盖或删除原件。 |

人工增强每次最多使用一个来源：人机验证类为 Unsplash 或 Pexels（含 Pexels Videos）；登录类为小红书、Instagram、YouTube 或 Bilibili；也可输入其他参考网址。验证或登录只由用户在自己的可见浏览器完成。Pexels 的一次验证会连续用于照片与视频；用户拒绝或增强失败时，自动校准仍然继续。每种被请求的媒体至少需要两个本次真正可见或可播放的独立来源，自动与人工证据可以合并计算。
